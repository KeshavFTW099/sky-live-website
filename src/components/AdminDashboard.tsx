...
interface IntelligentMediaInputProps {
  value?: string;
...
function IntelligentMediaInput({ 
  value, 
  onChange, 
  placeholder, 
  onBrowseLibrary, 
  onUploadComplete 
}: IntelligentMediaInputProps) {
...
export default function AdminDashboard({ content, onUpdateContent, onGoHome }: any) {
  ...
  const handleAuthError = useCallback((res: any) => {
    if (res.status === 401 || res.status === 403) {
      signOut({ redirect: false });
  ...
  }, [isAuthenticated, status, fetchMedia, fetchEnquiries]);
  ...
  const handleFileUpload = (e, onComplete?: any) => {
  ...
  const handleDragOver = (e, index?: any) => {
    e.preventDefault();
  };
  ...
  const handleCatDragOver = (e, index?: any) => {
    e.preventDefault();
  };
  ...
  const handleLayoutDragOver = (e, index?: any) => {
    e.preventDefault();
  };
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '8px' }}>
                {Object.entries(editingProduct.specifications || {}).map(([key, val], sIdx) => (
                  <div key={sIdx} style={{ display: 'flex', gap: '8px', alignItems: 'center', background: 'rgba(255,255,255,0.02)', padding: '6px 10px', borderRadius: '4px' }}>
                    ...
                    <span style={{ fontSize: '13px', flexGrow: 1 }}>{val as any}</span>
                    <button
                      type="button"
                      style={{ color: '#EF4444', fontSize: '12px', background: 'none', border: 'none', cursor: 'pointer' }}
                      onClick={() => {
